...
echo "[+] Uploading img image to 's3://${S3_BUCKET_NAME}/${IMAGE_KEY}.img"
$AWS_CLI s3 cp ${DOWNLOAD_DIRECTORY}/${IMAGE_KEY}.img s3://${S3_BUCKET_NAME} --only-show-errors

echo "[+] Import snapshot to EC2"
IMPORT_SNAPSHOT_ID=$($AWS_CLI ec2 import-snapshot --disk-container Format=img,UserBucket="{S3Bucket=${S3_BUCKET_NAME},S3Key=${IMAGE_KEY}.img}" | jq -r .ImportTaskId)
...
$AWS_CLI s3 rm s3://${S3_BUCKET_NAME}/${IMAGE_KEY}.img
